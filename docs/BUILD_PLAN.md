...
| W3 | **Visual design** | **Final** for phones. Three layouts share the same screens: **phones** (under 500 px) fill the screen edge to edge; **tablets** (500–899 px) show the 390 px phone frame; **desktop** (900 px and up) uses a left sidebar instead of the tab bar, multi-column grids, centred dialogs instead of bottom sheets, a sign-in card, and a 720 px column for the player booking flow. Desktop changes are opt-in per screen via `useIsDesktop()` (`src/lib/useIsDesktop.ts`), so the phone layout stays pixel-identical. Code changes must not change how screens look; F0 was checked with before/after screenshots of 24 screens. |
